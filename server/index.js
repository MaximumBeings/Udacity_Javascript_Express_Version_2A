...
// NASA OPPORTUNITY
const sol = 1000;
const apiUrlOpp = `https://api.nasa.gov/mars-photos/api/v1/rovers/opportunity/photos?sol=${sol}&api_key=${apiKey}`;
console.log(apiUrlOpp);


// example API Opportunitycall
...
const apiUrlSpirit = `https://api.nasa.gov/mars-photos/api/v1/rovers/spirit/photos?sol=${sol}&api_key=${apiKey}`;
...
app.get('/spiritData', async (req, res) => {
    try {
        let image = await fetch(apiUrlSpirit)
            .then(res => res.json())
        res.send({ image })
        console.log(res);
    } catch (err) {
